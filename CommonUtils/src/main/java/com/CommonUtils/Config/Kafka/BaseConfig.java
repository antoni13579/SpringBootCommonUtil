...
使用Consumer.seek方法，重新回到该未ack消息偏移量的位置重新消费，这种可能会导致死循环，原因出现于业务一直没办法处理这条数据，但还是不停的重新定位到该数据的偏移量上。
	 ...
	 * */
	public static <V> KafkaTemplate<String, V> getKafkaTemplate(final DefaultKafkaProducerFactory<String, V> defaultKafkaProducerFactory) 
	{ return new KafkaTemplate<String, V>(defaultKafkaProducerFactory); }
	...
	/**用于kafka事务管理*/
	public static <V> KafkaTransactionManager<String, V> getKafkaTransactionManager(final DefaultKafkaProducerFactory<String, V> defaultKafkaProducerFactory)
	{ return new KafkaTransactionManager<String, V>(defaultKafkaProducerFactory); }
